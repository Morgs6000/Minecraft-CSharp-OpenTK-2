...
public class Window : GameWindow {
    ...
    private void ProcessInput(FrameEventArgs args) {
        ...
        Vector3 newPosition = eye + direction * speed * (float)args.Time;
    ...
    public List<AABB> collisionBoxes = new List<AABB>();
    ...
    public void LoadTile(int x, int y, int z) {
        ...
        Vector3 min = new Vector3(x0, y0, z0);
        Vector3 max = new Vector3(x1, y1, z1);

        collisionBoxes.Add(new AABB(min, max));
    }

    public bool CheckCollision(Vector3 position) {
        AABB playerBox = new AABB(position - new Vector3(0.5f, 0.5f, 0.5f), position + new Vector3(0.5f, 0.5f, 0.5f));
